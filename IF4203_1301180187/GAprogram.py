...
class Individu:
	...
	# Attribut class Individu adalah kromosom,
...
class Populasi():
	...
	# Attribut utama class Populasi adalah array of Individu,
	...
	def TotalFitness(self):
		for individu in self.arr_individu:
			self.sum_fitness += individu.fitness

	def sortPopulasi(self):													# Sorting berdasarkan fitness descending dengan selection sort
	...
	def Regenerasi(self):
		...
		while sizeInduk < size + (self.sum_pop * elite):
			...
			if (not self.isSama(sama,select)):
				if (random.uniform(0,1) <= (self.arr_individu[select].fitness / self.sum_fitness)):
		...
		while (size < sizeInduk*2):
			rand1 = random.randint(0,sizeInduk)								# random integer untuk menentukan yang jadi induk1
			rand2 = random.randint(0,sizeInduk)								# random integer untuk menentukan yang jadi induk2
			if (rand1 != rand2):
				induk1 = self.arr_individu[rand1]							# induk1 dibentuk dari individu di indeks rand1
				induk2 = self.arr_individu[rand2]							# induk1 dibentuk dari individu di indeks rand2
				anak1 = self.Copy(self.arr_individu[rand1])					# buat anak1 mirip dengan ortunya (tidak wajib)
				anak2 = self.Copy(self.arr_individu[rand2])					# buat anak2 mirip dengan ortunya (tidak wajib)
